...
colorButtons.addEventListener('click', function handleClick(event){
  let isButton = event.target.classList.contains("color-btn");
  if (isButton){
    const newColor = event.target.style.backgroundColor;
    colorBox.style.backgroundColor = newColor;
    colorBox.innerText = event.target.hasAttribute("title") ? event.target.title : newColor.toString();
  }
